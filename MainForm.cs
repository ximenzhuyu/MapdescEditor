...
public sealed class MainForm : Form
{
    ...
    private void ShowOperationGuide()
    {
        const string guide = """
            ...
            2. 选择 MapInfo.txt
            选择服务端 Mir200\Envir\MapInfo.txt。
            地图名称以 MapInfo.txt 中的配置为准。
            同一个 .map 文件可以对应多个地图名称。
            ...
            4. 调整坐标
            快捷微调：选中右侧备注后，使用键盘 ↑、↓、←、→ 调整坐标。
            ↑：向上移动 1 格。
            ↓：向下移动 1 格。
            ←：向左移动 1 格。
            →：向右移动 1 格。
            大小地图联动调整：按住 Alt 再按方向键。
            Alt+↑：模式 0 和模式 1 同时向上移动 1 格。
            Alt+↓：模式 0 和模式 1 同时向下移动 1 格。
            Alt+←：模式 0 和模式 1 同时向左移动 1 格。
            Alt+→：模式 0 和模式 1 同时向右移动 1 格。
            ...
            8. 保存
            点击“保存”或“保存到客户端”。
            当前内容将写入客户端 MapDesc1.dat。
            勾选“保存前自动备份”时，会先生成带时间戳的 .bak 文件。
            如果备份失败，程序会中止保存操作。
            “另存为…”用于保存到其他文件，不覆盖客户端文件。
            """;
